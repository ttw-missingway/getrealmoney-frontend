...
import { type Message, validateMessage } from "../models/message";
...
socket.on("messages", (messages) => {
    ...
    for (const m of limitedMessages){
        const validatedMessage = validateMessage(m);
...
export function sendMessage(content: string, id: string) {
    return new Promise((resolve: (value: SendSuccess) => void, reject: (value: SendError) => void) => {
